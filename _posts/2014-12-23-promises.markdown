...
The macro `WITH-DELAY` schedules a timer on a runloop and executes its
body once the timer expires. In this example, the last two forms are
executed only after the promise returned by `WITH-DELAY` is
resolved.

Contrast the behaviour above with that of `CL:PROGN` where subsequent
forms are executed immediately and the output will look like this:
...
though you shouldn't use it to write any serious code.

[^1]: Definition from [Wikipedia](http://en.wikipedia.org/wiki/Futures_and_promises)

[^2]: Almost all javascript code has been copied from [Domenic's post][domenic-promises]
